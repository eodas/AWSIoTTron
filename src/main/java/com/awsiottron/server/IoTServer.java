...
public class IoTServer extends Thread {
	...
	private jBRMSRules jbpmRules;
	private ServerSocket server = null;

	private final Logger logger = LoggerFactory.getLogger(IoTServer.class);

	public IoTServer(jBRMSRules jbpmRules, int port) {
		this.port = port;
		this.jbpmRules = jbpmRules;
		System.out.println("Drools-jBPM BRMSServer, Started Port: " + port);
	}
	...
	public void run() {
		try {
			...
			alive = false;
			System.err.println(
					"Error: Port " + port + " is already in use.\n" + "Please select another port for BRMSServer.");
		}
		while (alive) {
	...
	public void killServer() {
		alive = false;
		System.out.println("Drools-jBPM BRMSServer Port, Stopped");
		try {
			if (server != null) {
				server.close();
				server = null;
			...
			e.printStackTrace();
			System.err.println("BRMSServer kill error: " + e.toString());
		}
	}
}
